...
def query_orders_all_info_from_master(order_id):
    global uid, headers, address, token, place_pair

    url = f"{address}/api/v1/orderOtherService/orderOther/{order_id}"

    payload = {
        "loginId": uid,
    }

    response = requests.get(url=url, headers=headers, json=payload)
    if response.status_code != 200 or response.json().get("data") is None:
        print(f"query orders failed, response data is {response.text}")
        return None
...
def query_orders_all_info_from_slave(order_id):
    """
    通过 orther-other-service2 中查询
    返回(orderId, tripId) triple list for consign service
    :param headers:
    :return:
    """
    ...
    url = f"{address}/api/v1/orderOtherService2/orderOther/{order_id}"
...
async def main():
    ...
    # 买的 orderOther，否则走不了预想的流程
    task1 = asyncio.ensure_future(preserve_ticket(trip_id))
    ...
    if order_id != None:
        query_orders_all_info_from_master(order_id)
        query_orders_all_info_from_slave(order_id)
...
# query_orders_all_info_from_slave('3d3800e1-0ff2-4aa7-bcdd-1d335eb92c58')
asyncio.run(main())
# query_one_and_put_consign()